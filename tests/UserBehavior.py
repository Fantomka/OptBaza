...
class UserBehavior(TaskSet):
    ...
    def on_stop(self):
        self.client.post("/logout", {"username": "UserName"})

    @task(1)
    def get_provider(self):
        with self.client.get('/object/provider', catch_response=True) as response:
            if response.status_code == 200:
                if response.json() == test_json
                    response.success()
                else:
                    response.failure(f'Неверный список поставщиков, продаюших все товары')
            response.failure(f'код статуса {response.status_code}')

    @task(3)
    def get_lower_price(self):
        self.client.get("/object/lower")
